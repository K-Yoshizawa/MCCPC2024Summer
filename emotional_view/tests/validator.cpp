...
int main(int argc, char* argv[]){
    ...
    vector<int> r(N);
    for(int i = 0; i < N; ++i){
        x[i] = inf.readInt(MIN_XY, MAX_XY, format("x_%d", i + 1));
        inf.readSpace();
        y[i] = inf.readInt(MIN_XY, MAX_XY, format("y_%d", i + 1));
        inf.readSpace();
        int L = sqrt(x[i] * x[i] + y[i] * y[i]);
        r[i] = inf.readInt(MIN_r, L, format("r_%d", i + 1));
        inf.readEoln();
    }
    inf.readEof();
